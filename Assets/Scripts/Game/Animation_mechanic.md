...
잡고 있는 쪽을 다시 누른 입력은 애초에 `PressResult.HoldBody`로 분류되어 버퍼에 들어오지 않는다(§3-1).
...
복구되는 순간은 A3(`HoldEnter`)으로 들어간다. 즉 **플레이어가 아무것도 누르지 않았는데
진입 원샷과 Y 트윈이 나갈 수 있다.** 마스크가 빈 동안 Y가 자유로워 반대편을 쳐서 이동해 있었다면
그 자리에서 홀드 위치로 되돌아온다. 코드에서 추론한 것이고 플레이로 확인하지는 않았다.
